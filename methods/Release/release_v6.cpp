...
SolveResult SolveOneQuery(const Graph& graph, const Query& query)
{
    ...
    if (!group_count)
        return {0.0, true};
    if (group_count > 16)
        throw std::runtime_error("ReleaseV6 supports group count <= 16.");
    ...
    bool dual_complete = false;
    const long long dual_step_buy_work = 2LL * graph.m + graph.n;
    ...
    for (int size = 1; size <= half; ++size)
    {
        ...
        for (int mask = 1; mask < subset_count; ++mask)
        {
            ...
            if (!dual_complete)
            {
                dual_bank_work += row_search_work;
                while (!dual_complete && dual_bank_work >= dual_step_buy_work)
                {
                    dual_bank_work -= dual_step_buy_work;
                    AdvanceDual();
                }
                if (size > 2 && dual_complete)
                {
                    dual.ReleaseResidual();
                    packing_built = true;
                }
            ...
            if (size == 2)
            {
                ...
                ++completed_pair_rows;
                if (!packing_built && dual_complete &&
        ...
        if (size == 2 && !packing_built && dual_complete)
        {
            dual.ReleaseResidual();
            packing_built = true;
        }
