...
namespace gst::methods::test11
{
    SolveResult SolveOneQuery(const Graph& graph, const Query& query)
    {
        ...
        if (g >= 23)
        {
            throw std::runtime_error("Test11 currently supports group count <= 22.");
        }
        ...
        {
            result.stats.total_group_vertices += static_cast<int>(query.groups[gi].size());
            result.stats.max_group_size =
                std::max(result.stats.max_group_size, static_cast<int>(query.groups[gi].size()));
        ...
        auto group_dist_start = std::chrono::steady_clock::now();
        for (int gi = 0; gi < g; ++gi)
        {
        ...
        result.stats.group_dist_ms = std::chrono::duration<double, std::milli>(
            std::chrono::steady_clock::now() - group_dist_start).count();

        std::vector<std::vector<double>> group_pair(g, std::vector<double>(g, fp::kInf));
        auto group_pair_start = std::chrono::steady_clock::now();
        for (int a = 0; a < g; ++a)
        {
            for (int b = 0; b < g; ++b)
            {
        ...
        result.stats.group_pair_ms = std::chrono::duration<double, std::milli>(
            std::chrono::steady_clock::now() - group_pair_start).count();
        ...
        result.stats.greedy_ms = std::chrono::duration<double, std::milli>(
            std::chrono::steady_clock::now() - greedy_start).count();
        if (greedy_best < best)
        {
            best = greedy_best;
        }
