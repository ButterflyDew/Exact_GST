...
struct TinyDsu
{
    std::array<int, 24> parent{};

    explicit TinyDsu(int n)
    {
        for (int i = 0; i < n; ++i)
            parent[i] = i;
    }
...
bool EnvFlagEnabled(const char* name, bool default_enabled = false)
{
...
{
    return EnvFlagEnabled("GST_TEST19_PROGRESS");
}

bool AstarOrderingEnabled()
{
    return EnvFlagEnabled("GST_TEST19_ASTAR_ORDER");
}

bool OneTreeLbDiagnosticEnabled()
{
    return EnvFlagEnabled("GST_TEST19_ONETREE_LB_DIAG");
}

bool TspLbDiagnosticEnabled()
{
    return EnvFlagEnabled("GST_TEST19_TSP_LB_DIAG");
}

bool TspLbOrderingEnabled()
{
    return EnvFlagEnabled("GST_TEST19_TSP_LB_ORDER", true);
}

bool TspLbSaveEnabled()
{
    return EnvFlagEnabled("GST_TEST19_TSP_LB_SAVE", true);
}

int OneTreeLbPruneMode()
{
    const char* value = std::getenv("GST_TEST19_ONETREE_LB_PRUNE");
    if (!value || !value[0])
        return 0;
    std::string_view flag(value);
    if (flag == "0" || flag == "false" || flag == "FALSE" || flag == "off" || flag == "OFF")
        return 0;
    if (flag == "seed" || flag == "SEED")
        return 2;
    if (flag == "final" || flag == "FINAL")
        return 3;
    return 1;
}
...
SolveResult SolveOneQuery(const Graph& input_graph, const Query& input_query)
{
    ...
    if (!g)
        return {0.0, true, {}};
    if (g > 22)
        throw std::runtime_error("Test19 supports group count <= 22.");
    ...
    const bool tsp_lb_diag = TspLbDiagnosticEnabled();
    const bool tsp_lb_order = TspLbOrderingEnabled();
    const bool tsp_lb_save = TspLbSaveEnabled();
    const bool tsp_lb_enabled = tsp_lb_diag || tsp_lb_order || tsp_lb_save;
    MetricTspLowerBound tsp_lb_index;
    if (tsp_lb_enabled)
    {
        const auto tsp_precompute_begin = Clock::now();
        tsp_lb_index.Build(gp, U);
        st.tsp_lb_precompute_ms =
            std::chrono::duration<double, std::milli>(Clock::now() - tsp_precompute_begin).count();
    }
    auto OneTreeAllAnchorHalf = [&](int root,
                                    const int* bits,
                                    int cnt,
                                    const std::vector<MetricEdge>& group_edges)
    {
        if (cnt <= 0)
            return 0.0;
        if (cnt == 1)
            return gd[bits[0]][root];

        const int node_count = cnt + 1;

        std::array<double, 24> root_w{};
        root_w.fill(fp::kInf);
        std::vector<MetricEdge> root_edges;
        root_edges.reserve(cnt);
        for (int i = 0; i < cnt; ++i)
        {
            root_w[i + 1] = gd[bits[i]][root];
            root_edges.push_back({0, i + 1, root_w[i + 1]});
        };
        ...
        auto Incident = [](int anchor, const MetricEdge& edge)
        {
            return edge.u == anchor || edge.v == anchor;
        };
        auto EdgeWeight = [&](int a, int b)
        {
            ...
            if (b == 0)
                return root_w[a];
            return gp[bits[a - 1]][bits[b - 1]];
        };

        double best_bound = 0.0;
        for (int anchor = 0; anchor < node_count; ++anchor)
        {
            double first = fp::kInf, second = fp::kInf;
            for (int v = 0; v < node_count; ++v)
            {
                if (v == anchor)
                    continue;
                double w = EdgeWeight(anchor, v);
                if (w < first)
            ...
            best_bound = std::max(best_bound, (mst_sum + first + second) * .5);
        }
        return best_bound;
    };
    ...
    const bool onetree_lb_diag = OneTreeLbDiagnosticEnabled();
    const int onetree_lb_prune_mode = OneTreeLbPruneMode();
    const bool onetree_lb_prune_seed = onetree_lb_prune_mode == 1 || onetree_lb_prune_mode == 2;
    const bool onetree_lb_prune_final = onetree_lb_prune_mode == 1 || onetree_lb_prune_mode == 3;
    st.astar_order_enabled = astar_order ? 1 : 0;
    st.onetree_lb_diag_enabled = onetree_lb_diag ? 1 : 0;
    st.onetree_lb_prune_enabled = onetree_lb_prune_mode;
    st.tsp_lb_diag_enabled = tsp_lb_diag ? 1 : 0;
    st.tsp_lb_order_enabled = tsp_lb_order ? 1 : 0;
    st.tsp_lb_save_enabled = tsp_lb_save ? 1 : 0;
    if (verbose_progress)
        std::cerr << "[Test19] preprocess done elapsed="
    ...
    std::vector<double> onetree_cache(N), tsp_cache(N);
    std::vector<int> cov(N), touched, kept, lb_seen(N), far_seen(N), near_seen(N);
    std::vector<int> onetree_seen(N), tsp_seen(N);
    ...
    for (int s : order)
    {
        ...
        std::vector<MetricEdge> onetree_group_edges;
        bool onetree_group_edges_ready = false;
        auto EnsureOneTreeGroupEdges = [&]()
        {
            if (onetree_group_edges_ready)
                return;
            onetree_group_edges.reserve(rem_cnt * (rem_cnt - 1) / 2);
            for (int i = 0; i < rem_cnt; ++i)
                for (int j = i + 1; j < rem_cnt; ++j)
                    onetree_group_edges.push_back({i + 1, j + 1, gp[rem_bits[i]][rem_bits[j]]});
            std::sort(onetree_group_edges.begin(), onetree_group_edges.end(),
                      [](const MetricEdge& a, const MetricEdge& b)
                      {
                          if (a.w != b.w)
                              return a.w < b.w;
                          if (a.u != b.u)
            ...
            onetree_group_edges_ready = true;
        };
        ...
        auto OneTreeLb = [&](int v)
        {
            if (onetree_seen[v] == stamp)
                return onetree_cache[v];
            double ans = Lb(v);
            if (rem_cnt >= 2)
            {
                EnsureOneTreeGroupEdges();
                ans = std::max(ans, OneTreeAllAnchorHalf(v, rem_bits, rem_cnt, onetree_group_edges));
            }
            onetree_seen[v] = stamp;
            return onetree_cache[v] = ans;
        };
        auto TspLb = [&](int v)
        {
            if (tsp_seen[v] == stamp)
            ...
            double ans = Lb(v);
            if (rem_cnt >= 2)
                ans = std::max(ans, tsp_lb_index.TourHalf(rem, v, gd));
            tsp_seen[v] = stamp;
            ++st.tsp_lb_queries;
            st.tsp_lb_ms += std::chrono::duration<double, std::milli>(Clock::now() - begin).count();
            return tsp_cache[v] = ans;
        };
        auto DiagnoseOneTreeLb = [&](int v, double d, double base_lb, double need_lb, bool seed_site)
        {
            if (!onetree_lb_diag)
                return;
            const auto begin = Clock::now();
            const double one_lb = OneTreeLb(v);
            ++st.onetree_lb_diag_checks;
            if (one_lb > base_lb + 1e-9)
                ++st.onetree_lb_diag_stronger_lb;
            if (one_lb > need_lb + 1e-9)
            {
                ++st.onetree_lb_diag_stronger_need;
                const double gain = one_lb - need_lb;
                st.onetree_lb_diag_gain_sum += gain;
                st.onetree_lb_diag_gain_max = std::max(st.onetree_lb_diag_gain_max, gain);
                if (d + need_lb <= best && d + one_lb > best)
                {
                    if (seed_site)
                        ++st.onetree_lb_diag_extra_seed_prune;
                    else
                        ++st.onetree_lb_diag_extra_final_prune;
                }
            }
            st.onetree_lb_diag_ms +=
                std::chrono::duration<double, std::milli>(Clock::now() - begin).count();
        };
        auto DiagnoseTspLb = [&](int v, double d, double base_lb, double need_lb, bool seed_site)
        {
            if (!tsp_lb_diag)
                return;
            const auto begin = Clock::now();
            const double tsp_lb = TspLb(v);
            ++st.tsp_lb_diag_checks;
            if (tsp_lb > base_lb + 1e-9)
                ++st.tsp_lb_diag_stronger_lb;
            if (tsp_lb > need_lb + 1e-9)
            {
                ++st.tsp_lb_diag_stronger_need;
                const double gain = tsp_lb - need_lb;
                st.tsp_lb_diag_gain_sum += gain;
                st.tsp_lb_diag_gain_max = std::max(st.tsp_lb_diag_gain_max, gain);
                if (d + need_lb <= best && d + tsp_lb > best)
                {
                    if (seed_site)
                        ++st.tsp_lb_diag_extra_seed_prune;
                    else
                        ++st.tsp_lb_diag_extra_final_prune;
                }
            }
            st.tsp_lb_diag_ms +=
                std::chrono::duration<double, std::milli>(Clock::now() - begin).count();
        ...
        auto FinalSaveNeedLowerBound = [&](int v, double base_lb)
        {
            return tsp_lb_save ? std::max(base_lb, TspLb(v)) : base_lb;
        };
        ...
        Heap q;
        const bool order_with_lb = astar_order || tsp_lb_order;
        auto OrderLb = [&](int v)
        {
            return tsp_lb_order ? TspLb(v) : Lb(v);
        };
        auto QueueKey = [&](int v, double d)
        {
            return order_with_lb ? d + OrderLb(v) : d;
        };
        for (int v : touched)
        {
            double d = dist[v];
            ...
            DiagnoseOneTreeLb(v, d, old_lb, old_lb, true);
            DiagnoseTspLb(v, d, old_lb, old_lb, true);
            if (onetree_lb_prune_seed)
            {
                const double one_lb = OneTreeLb(v);
                if (one_lb > old_lb + 1e-9 && d + one_lb > best)
                {
                    ++st.onetree_lb_seed_pruned;
                    continue;
                }
            }
            double queue_lb = old_lb;
            if (tsp_lb_order)
            {
                const double tsp_lb = TspLb(v);
                if (tsp_lb > old_lb + 1e-9 && d + tsp_lb > best)
                {
                    ++st.tsp_lb_order_seed_pruned;
                    continue;
                }
                queue_lb = tsp_lb;
            }
            q.push({order_with_lb ? d + queue_lb : d, v});
        ...
        while (!q.empty())
        {
            ...
            if (tsp_lb_order)
            {
                const double tsp_lb = TspLb(u);
                if (tsp_lb > old_lb + 1e-9 && d + tsp_lb > best)
                {
                    ++st.tsp_lb_order_pop_pruned;
                    continue;
                }
            }
            Complete(u, d);
            ...
            for (auto e : graph.adj[u])
            {
                ...
                if (order_with_lb)
                {
                    double next_lb = OrderLb(e.to);
                    if (nd + next_lb > best)
                    {
                        if (tsp_lb_order)
                            ++st.tsp_lb_order_relax_pruned;
                        else
        ...
        for (int v : kept)
        {
            ...
            const double save_need_lb = SaveNeedLowerBound(v);
            const double final_save_need_lb = FinalSaveNeedLowerBound(v, save_need_lb);
            double need = dist[v] + final_save_need_lb;
            if (full_lb[v] > best)
            {
                ++st.final_pruned_full;
                continue;
            }
            DiagnoseOneTreeLb(v, dist[v], base_need - dist[v], save_need_lb, false);
            DiagnoseTspLb(v, dist[v], base_need - dist[v], save_need_lb, false);
            if (need > best)
            {
                ++st.final_pruned_need;
                if (tsp_lb_save && final_save_need_lb > save_need_lb + 1e-9 &&
                    dist[v] + save_need_lb <= best)
                    ++st.tsp_lb_save_pruned;
            ...
            if (onetree_lb_prune_final)
            {
                const double one_lb = OneTreeLb(v);
                if (one_lb > save_need_lb + 1e-9 && dist[v] + one_lb > best)
                {
                    ++st.onetree_lb_final_pruned;
                    continue;
                }
            }
            kept[keep_w++] = v;
        ...
        {
            const double save_need_lb = SaveNeedLowerBound(v);
            double need = dist[v] + FinalSaveNeedLowerBound(v, save_need_lb);
            if (z.light)
            {
                if (z.dense_row)
                    z.dense[v] = dist[v];
