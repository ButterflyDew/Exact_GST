...
{
    SolveResult result;
    Test21Stats& stats = result.stats;
    stats.n = graph.n;
    stats.m = graph.m;
    ...
    if (stats.g > 16)
        throw std::runtime_error("Test21 ordered-row prototype supports group count <= 16.");
    ...
    dual.Build(graph, query, group_distance, root_star_root);
    ...
    for (int size = 1; size <= stats.half - 1; ++size)
    {
        for (int mask = 1; mask < subset_count; ++mask)
        {
            ...
            auto Set = [&](int vertex, double value)
            {
                if (value >= row_distance[vertex] || value + BaseH(vertex) > best)
                    return;
                if (row_distance[vertex] == fp::kInf)
                    touched.push_back(vertex);
                row_distance[vertex] = value;
            ...
            while (!queue.empty())
            {
                ...
                {
                    const double next = node.distance + edge.w;
                    if (next >= row_distance[edge.to] || next + BaseH(edge.to) > best)
                        continue;
                    if (row_distance[edge.to] == fp::kInf)
                    {
                        touched.push_back(edge.to);
