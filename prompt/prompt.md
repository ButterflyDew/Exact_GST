...
新建目录，hacktests，在其中写程序随机生成图和询问都较小的数据，运行 DPBF 和 test4 的代码，直到它们输出的答案权值不同。

具体的，可以默认图为 n=12 的稀疏整数权图，询问次数只有 1 且 g=6 的图，默认比较的两个代码是 DPBF 和 test4，这里可以在代码中留好接口和注释方便我后续修改。