# Distance-only Epoch Rows：低空间表示历史

更新时间：2026-07-10。本文说明独立探针 `tools/distance_epoch_solver_probe`，它从 ReleaseV1 复制出一条无开关执行路径，用于验证两个相互配合但逻辑独立的机制：

> 后续更新：该工具现增加 `--budget`、`--dynamic-dual` 与 `--dual-budget` 三个研究入口，用于核算 offline -> dual -> global 的无参数升级点；默认求解路径和本文冻结结果不变。新设计与数据统一见 `half_global_hybrid.md` 和 `../archive/half_global_probe_log_20260710.md`。

> 状态更新：本文记录 dual-cut 之前的表示层基底。其后旧 distance solver 加入 potential 仍未完成 full；ReleaseV2 首次完成，ReleaseV3 再把 ordered rows 与 global 有机接续。本文中的 `30.749s` fast 与 900 秒结果只代表冻结阶段，当前结论见 `../release_v3.md`。
