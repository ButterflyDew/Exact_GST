...
这份文件给无记忆 agent 快速接手 Test18。完整历史实验记录保存在 `readme_files/test18_research_log.md`；本文件前半部分先说明当前实现、正确性边界、验证状态与下一步优先级，后半部分保留较详细的原始说明。

## 0. 当前状态一句话

Test18 是当前大组数组斯坦纳树实验主线：它保留精确 half-DP 框架，只用安全下界剪枝、只用合法完整解更新上界；当前真正有效的增量是 tree-aware greedy、虚拟 singleton、full-root lower-bound corridor、stale `need` compact、dense-light 行表示、cover-aware Complete，以及 k=2 后的 pair/single 同根分区上界。

DBLP g15 仍未跑通。现有 best 已能在 pair 层完成后从 `19.3812` 降到 `17.7702`，但 k=3 行仍快速变稠密；下一步重点应继续减少 dense row 的空间/扫描，或寻找更本质的状态保存必要条件，而不是继续堆小幅 LB。
...
2. sparse-light：保存 `v,d`，用于 pair 等不保留 cover/need 的轻量行；
...
| dense-light | 稠密行少存 `cover/need` | 仍保存精确距离；丢 cover 只少做 upper update | pair/k3 空间常数明显下降 |
...
这些方向不要无意识重做；详细数据见 `test18_research_log.md`。

- k=3 后 singleton/pair/triple root partition：正确但 Toronto g12 额外约 `428ms` 且不更新 best。
...
- 随机对拍多轮通过；
- Toronto 默认 query 1 与已有 DPBF 结果一致；
- DBLP snapshot g9 query 1 权重 `11.8766830000`；
- pair/single 分区在 DBLP snapshot g9 上把 finite states 从约 `3.515M` 降到 `1.808M`。

本轮瘦身后的验证：

- 随机小图对拍：`seed=202020`，80 组通过；
- Toronto 默认 query 1：Test18 `0.2582152999`，已有 DPBF `0.2582152999`；
- DBLP snapshot large / `DBLP_data_bfs` g9 query 1：`11.8766830000`，`finite_states=1.807961M`，`dense_rows=36`，`wall_ms≈4365`，`peak_rss≈74.9 MiB`；
- Test18 输出已删除长期为 0 的 `h_*`、`certify_*`、`confirmed_states`、`valid_total/valid_k*` 和撤回后的 `packed_dense_*` 字段。
...
1. 做 full DBLP g15 的 k=2 完整层与 k=3 初段内存斜率探针，确认当前瘦身版与旧主线一致。
2. 继续寻找“减少 dense 行保存范围”的必要条件，而不是只换 dense 存储布局。
3. 不建议继续小幅增强 LB：pair slack 诊断显示 DBLP pair 状态离 best 门槛普遍很远，小幅 LB 很难产生数量级状态下降。
