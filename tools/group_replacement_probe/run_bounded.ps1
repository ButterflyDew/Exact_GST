...
$exe = Join-Path $workspace "build\tools\group_replacement_probe\Release\gst_group_replacement_probe.exe"
...
$arguments = @(
    $(if ($Mode -eq "pair") { "--pair-layer-signal" } else { "--seed-signal" }),
    $DataRoot,
    $GraphSelector,
    $QuerySelector,
